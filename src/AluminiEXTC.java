...
public class AluminiEXTC{
	...
	public AluminiEXTC() {
		...
		JFrame frame11= new JFrame("Alumini List:Electronics Engineering");
		frame11.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame11.setBounds(0, 0, 1200, 750);
		frame11.setIconImage(image.getImage());
		...
		frame11.getContentPane().add(panel);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(table.getSelectedRowCount()==1) {
					...
					try {
						...
						String sql = "DELETE FROM aluminiinfo_extc where id = "+cell;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(table.getSelectedRowCount()==1)
				{
					...
					try {
						Class.forName("com.mysql.cj.jdbc.Driver");
						Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/miniproject","root","Bappamorya@1133");
						String query="Update aluminiinfo_extc set Full_Name='"+name+"',Passout_Year='"+passyr+"',Number='"+phno+"',Email_Id='"+emlid+"' where Full_Name='"+name+"'";
		...
		btnShowData.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String sql="Select * from aluminiinfo_extc";
				...
				if(model.getRowCount()==0) {
					...
					try {
						...
						for(int i=0;i<model.getRowCount();i++) {
							...
							String query = "insert into aluminiinfo_extc(Full_Name,Passout_Year,Number, Email_Id) values(?,?,?,?)";
		...
		frame11.getContentPane().add(panel_2);
